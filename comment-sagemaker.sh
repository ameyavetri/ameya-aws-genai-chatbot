#!/bin/bash
# comment-sagemaker-models.sh
# Comments out SageMaker code while preserving it for future use

echo "🔧 Commenting out SageMaker models (preserving for future)..."

# Backup original
cp lib/models/index.ts lib/models/index.ts.backup

cat > lib/models/index.ts << 'ENDOFFILE'
...
echo "✅ SageMaker code preserved and commented out"
echo ""
echo "📋 To enable SageMaker models in the future:"
echo "1. Uncomment the import section (lines 16-23)"
echo "2. Uncomment the desired model sections"
echo "3. Update config.json with model names"
echo "4. npm run build && cdk deploy --all"
echo ""
echo "🔨 Building..."
npm run build